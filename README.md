...
Replay defaults to `public/trajectories/pingpongfsm_play_20s.json`. Use `Upload NPZ` in the UI to load recorder `.npz` files directly in the browser; converted `.json` files are also accepted. The upload contract is documented in [`docs/replay_protocol.md`](docs/replay_protocol.md).
